...
for x in `ls | grep .crt$`; do
  ...
  if [ $today -ge $expiredate ]; then
    ...
    ssh -v $caserver "openssl ca -config /root/ca/intermediate/openssl_intermediate.cnf -extensions server_cert -days 825 -notext -md sha512 -in /root/ca/intermediate/csr/$domain.csr -out /root/ca/intermediate/certs/$domain.crt -passin pass:${CERT_PASSWORD} -batch" 
